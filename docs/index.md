...
| 보던 문서 위치나 웹페이지로 다시 돌아가고 싶어요 | **[업무 책갈피 0.2.4 평가판 받기](tools/bookmark/index.md)** — Ctrl+Alt+B로 남기고 목록이나 포스트잇 스티커에서 이어가세요. |

FolderState 0.1.3 RC1은 폴더 확인과 아이콘 위치 적용을 분리합니다. Excel 명단 비교 R12는 차이 결과와 담은 목록 확인의 문구·표를 간결하게 정리했습니다. 업무 책갈피 0.2.4 평가판은 사용자 지정 설치 경로의 복구·제거를 수정했습니다. 기존 포스트잇에서 메모를 편집하다 다른 창으로 이동하면 자동 저장합니다. 스티커에는 저장 버튼이 없습니다. 선택범위 내보내기 평가판은 Excel 시작 시 메뉴를 준비하고, 사용자가 누를 때만 새 통합문서를 만듭니다. File List to Excel 1.2.0은 목록·중복 찾기와 선택 설치인 Excel 파일 복사를 제공합니다. 다운로드 전에 각 설치 안내에서 사용 환경과 알려진 제한을 확인하세요.
