...
2026-09-26 후속 상태: 초기 0.1.1 build02는 실제 업그레이드와 두 UI 사용 후 종료를 통과했다. 그러나 상태바 복원 보완 후 후보 `8741b19f…`는 자동 로드·붙여넣기 성공 뒤 정상 종료에서 BEX64/c0000409가 재현됐다. 새 덤프가 없어 원인은 미확정이며 이전 PASS와 새 FAIL을 함께 보존한다. 후속 전용 이벤트 sink·Application 소유권·ABI·활성 callback 종료 보완은 전체 9종 284개 검사와 외부 시험 엔진의 실제 60개 회귀·정확한 정리 검증, 생산 DLL의 R17을 통과했다. 잔여 Excel을 정리한 뒤 생산 DLL `7080305a…`의 실제 설치·정상 자동 로드와 붙여넣기 및 별도 붙여넣기·즉시 Undo 후 두 자연 종료도 확인했다. 각각 지연 관찰에서 프로세스가 없고 새 Excel 오류 이벤트·WER가 없었다. 같은 DLL의 제거·제거 후 메뉴 부재·재설치와 붙여넣기 뒤 숨김 값 보존 및 반복 종료도 확인했다. PR #7은 main에 병합했으며 최종 검증 문서·CI·패키지·공개는 별도 게이트이다. 범위는 [ADR-0013](0013-visible-cells-paste-com-lifetime.md) 및 [시험 보고서](../../tools/VisibleCellsPaste/docs/test-report.md)를 따른다.
