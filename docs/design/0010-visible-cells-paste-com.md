...
상태: 채택 · 독립 COM 구조 유지. 0.1.1 참조 수명 수정과 후속 검증은 ADR-0013 참조; 미검증 환경과 조직 승인은 별도
날짜: 2026-09-24  
결정 담당: 도구 개발·검증 담당  
...
대체·대체됨 관계: 독립 COM 구조는 유지. [ADR-0013](0013-visible-cells-paste-com-lifetime.md)이 게시 후 발견된 종료 실패에 대응하는 참조 수명 수정과 재검증 게이트를 보완함. 다른 제품의 설계·설치 식별자는 변경하지 않음.

2026-09-24 후속 상태: 0.1.0 실제 사용 후 종료 실패가 3회 확인됐으며, 0.1.1 수명 수정의 실제 Excel 종료 검증은 아직 수행하지 않았다. 0.1.0 공개 릴리스는 초안으로 전환해 보류했고 태그는 유지했다. 아래 최초 결정과 시험 기록을 현재 종료 안전성의 보증으로 해석하지 않는다.

2026-09-26 후속 상태: 초기 0.1.1 build02는 실제 업그레이드와 두 UI 사용 후 종료를 통과했다. 그러나 상태바 복원 보완 후 후보 `8741b19f…`는 자동 로드·붙여넣기 성공 뒤 정상 종료에서 BEX64/c0000409가 재현됐다. 새 덤프가 없어 원인은 미확정이며 이전 PASS와 새 FAIL을 함께 보존한다. 후속 전용 이벤트 sink·Application 소유권·ABI·활성 callback 종료 보완은 전체 9종 284개 검사와 외부 시험 엔진의 실제 60개 회귀·정확한 정리 검증, 생산 DLL의 R17을 통과했다. 새 설치본의 정상 사용 후 종료는 아직 미검증이다. 잔여 Excel로 새 설치 게이트도 차단됐다. main 반영·최종 패키지·공개는 별도 게이트이며 [ADR-0013](0013-visible-cells-paste-com-lifetime.md) 및 [시험 보고서](../../tools/VisibleCellsPaste/docs/test-report.md)를 따른다.
