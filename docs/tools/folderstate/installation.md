...
v0.1.0은 상용 정식판 승인 전의 검증용 배포입니다. 현재 패키지는 코드 서명이 없으며 게시자·지원 정책과 별도 깨끗한 PC의 설치·복구·제거 확인이 완료되지 않았습니다. 조직에서 사용 가능한지 먼저 확인하고 시험용 폴더에서 시작합니다. 이 사이트에서는 MSI 설치 파일을 제공하지 않습니다.
...
새 버전의 MSI를 실행하면 기존 설치를 교체합니다. 이전 버전으로의 설치는 차단합니다.
