...
설치→자동 로드→사용→재시작→재설치→제거 및 화면 캡처 퀵가이드는 **BLOCKED_POLICY / BLOCKED_ENV**다. 최종 XLAM이 없으므로 설치 가능한 제품의 공개 릴리즈로 게시하지 않았다. [저장소 릴리즈 목록](https://github.com/prozac0401/Workspace/releases)의 별도 Excel draft(`excel-smart-list-compare-v0.2.0-rc.1`)에 검토용 소스 후보 ZIP과 해시만 업로드했다. 이 draft는 저장소 권한이 있는 사용자만 볼 수 있으며 공개 제품 릴리즈가 아니다.
...
후속 소스 후보는 `ExcelSmartListCompare_v0.2_SourceCandidate_BLOCKED_POLICY_20260914_r2.zip`이다. SHA-256은 `ad2ee868751e20340a851a7db18050cf433a2cfca9783525743281abf1f2107d`다. 78개 파일의 ZIP CRC 및 내부 해시 목록을 검증했고, XLAM과 원시 레지스트리·계정 로그는 포함하지 않았다. Excel draft를 커밋 `3297d5a595a5130eeb6a2d325178a0ac21650a67` 기준의 후속 소스 후보와 해시로 갱신했다. GitHub 자산 digest 2개 모두 로컬과 일치했다. 이 후보 역시 설치 가능한 Release를 대신하지 않는다.
