...
## Excel RC6 · 파일 창 전환 상태와 이전 버전 교체

2026-09-15 두 파일 간 메뉴가 초기 상태로 남는 사용자 보고를 같은 Excel에서 재현했다. 첫 번째 목록 데이터는 유지됐으나 창별 메뉴가 갱신되지 않았다. 창 활성화와 우클릭 직전에 현재 창의 메뉴를 동기화하고, 사용자가 선택한 **두 번째 목록 담아 비교**를 적용했다.

설치 시 이전 버전을 감지해 제거한 뒤 새 버전을 설치하는 후속 요구도 포함한다. 새 Install.cmd 한 번으로 제품 파일과 자동 실행 등록을 교체하고, 처리 가능한 실패에서는 이전 상태를 복구한다. 같은 버전은 다시 설치하고 더 최신 버전·알 수 없는 기록은 보존한다. 격리 거래 검사와 실제 RC5 → RC6 업데이트·실패 복구는 [업데이트 검증 기록](../../tools/ExcelSmartListCompare/docs/UPGRADE_REPORT.md)을 따른다.
