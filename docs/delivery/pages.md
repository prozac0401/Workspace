...
현재 공개 저장소에는 정책·가상 양식·명세·문서 원본과 문서 빌드 파일을 게시합니다. FolderState 애플리케이션 소스와 MSI는 로컬 Workspace에서 관리합니다. 로컬 업무 경로·메일·명단·사용자 로그·인증서·설치 진단 파일은 포함하지 않습니다. site/ 출력 전체가 공개된다는 기준으로 검토합니다.
