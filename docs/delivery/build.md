...
powershell -NoProfile -File scripts/build.ps1 -Version 0.1.3
```

build.ps1은 엔진 통합 테스트와 WPF 상호작용 회귀 시험(verify)·화면 렌더 → WPF·CLI 자체 포함 publish → 고정 설치 파일 목록 생성 → WiX MSI 생성 → SHA-256 생성 순서로 실행합니다. 검증용 옵션 -SkipTests는 테스트를 건너뛴 사실을 별도로 기록할 때만 사용합니다.
