...
사용자는 EXE 하나를 다운로드하고 설치 버튼을 누른다. 기존 RC1~RC6에서 업데이트하거나 RC7을 재설치하는 판단은 원래 엔진이 수행한다. 원래 ZIP과 소스 자산은 보존한다. PowerShell 의존과 조직 정책의 영향은 남으며 전체 매크로 설정이나 영구 실행 정책을 변경하지 않는다. EXE는 Windows x64용이고 코드 서명이 없는 배포다.
