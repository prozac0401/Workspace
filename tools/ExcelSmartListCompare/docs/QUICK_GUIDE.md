...
버전: 0.2.0 RC4 평가용 · Excel 화면 확인 환경: RC3의 Windows 11 / 데스크톱 Excel 64비트

선택한 첫 범위를 기준으로 담고 다음 범위와 비교합니다. 가로·세로·비교 모드를 선택할 필요가 없습니다. 화면은 2026-09-14~15 실제 Excel과 합성 시험 파일에서 촬영했습니다. RC3 설치 후 자동 시작·기준 담기·지연 보호 화면을 새로 촬영했으며, 나머지는 동일 VBA 엔진의 RC1 촬영본입니다.

## 1. 설치

1. Excel용 Release ZIP을 내려받아 **모두 압축 풀기**를 합니다.
2. 업무를 저장하고 Excel 창을 모두 닫습니다.
3. 풀린 `Release` 폴더의 **Install.cmd를 더블클릭**하고 이 제품 설치를 확인합니다. RC4는 PowerShell 명령 입력 없이 설치 준비를 처리합니다.
...
![RC3 일반 데스크톱에서 실제 버튼을 눌러 기준 5건이 저장된 화면](images/14-rc3-capture-a.jpg)
...
100,000셀은 입력 안전 상한이며 모든 명단을 끝까지 처리한다는 보장은 아닙니다. 실제 시험에서 10만 건씩 모두 다른 명단은 활성 처리 약 30초의 지연 보호로 중단됐습니다. 범위를 나누어 비교하세요.
...
지원 범위와 최신 실제 시험·복구 결과는 [RC3 강건성 보고서](WINDOWS_ROBUSTNESS_REPORT.md)를 확인하세요. [RC2](WINDOWS_TRUST_LOCATION_REPORT.md)·[RC1](WINDOWS_E2E_REPORT.md)은 이전 실행 기록입니다. Excel VBA 실행 이후 기본 실행 취소(Undo) 기록 보존은 보장하지 않습니다.
