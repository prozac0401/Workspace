...
버전: 0.2.0 RC8 · 기존 캡처 환경: RC1·RC3의 Windows 11 / 데스크톱 Excel 64비트

현재 RC8은 **Esc 실기 검사 실패로 배포 보류** 상태입니다. 아래는 기능·시험용 사용 안내이며, [실제 판정과 제한](RC8_COMPLETION_REPORT.md)을 먼저 확인하세요.
...
아래 실제 캡처는 문구 변경 전 RC1·RC3 화면입니다. 캡처의 ‘기준’ 및 ‘A/B’는 현재의 ‘첫 번째 목록 / 두 번째 목록’에 해당합니다. RC7의 실제 메뉴·대화상자와 실패 이력은 [잠금 해제 실기 기록](UNLOCKED_UI_REPORT.md)에 보존합니다. 수정된 RC8 파일의 현재 시험 결과는 [RC8 검증 기록](RC8_COMPLETION_REPORT.md)을 따릅니다.
...
**이전 버전이 있어도 새 Release의 Install.cmd만 실행하세요.** RC8은 설치된 RC1~RC7를 감지하고, 한 번의 설치 확인으로 이전 제품 파일과 자동 실행 등록을 제거한 뒤 새 버전을 설치합니다. 같은 버전은 다시 설치합니다. 설치 도중 오류가 나면 이전 파일과 등록을 복구하며, 더 최신 버전이나 알 수 없는 설치 기록은 덮어쓰지 않습니다. [현재 업데이트 검증](RC8_COMPLETION_REPORT.md)
...
두 번째 파일의 창으로 옮겨 비교할 범위를 선택합니다. 두 번째 파일이 아직 열려 있지 않다면 Excel에서 엽니다. 예시는 세로 범위 `B2:B6`입니다.
...
현재 메뉴는 열 때마다 첫 번째 목록을 확인하고 **두 번째 목록 담아 비교**, 개수, 비우기·바꾸기를 구성합니다. 여러 영역은 Ctrl로 선택할 수 있으며 선택 전체가 하나의 목록입니다. 사용 안내에서 `0.2.0-rc.8`로 설치 버전을 확인할 수 있습니다.

첫 번째 목록은 같은 Excel 실행 세션에서만 공유합니다. 파일을 여는 두 방식 모두에서 메뉴가 다르다는 후속 보고가 있어 RC7에서 메뉴 구성을 변경했습니다. 이번 Windows·Excel 환경의 두 경로에서는 실제 메뉴가 같은 상태를 표시했습니다. 이벤트를 끈 조건에서는 우클릭 메뉴가 최신 개수를 표시해도 기존 추가 기능 도구 모음의 개수가 늦게 갱신되는 현상이 관찰됐습니다.
...
처리 중에는 원본을 편집하지 말고 기다리세요. **Esc**로 취소를 요청할 수 있습니다. Excel 내부 호출 중에는 제어권이 돌아올 때 처리하므로 큰 목록은 범위를 나눠 실행하세요. 실제 입력의 확인 범위는 [RC8 검증 기록](RC8_COMPLETION_REPORT.md)을 따릅니다. 아래 RC1 취소 화면은 이전 버전의 기록입니다.

![RC1에서 Esc로 실제 취소한 화면](images/08-cancelled.jpg)

병합 셀, 너무 긴 텍스트, 안전 상한을 넘는 선택은 중단합니다. 범위를 줄여 다시 실행하세요. 실패·취소 시 담아 둔 첫 목록과 원본을 보존하며, 기존 결과를 덮어쓰지 않습니다. RC7의 실패·상태표시줄 관찰 기록과 RC8의 재검증 결과는 구분합니다.
...
제거는 압축을 푼 Release 폴더의 **Uninstall.cmd**를 실행하고 제품 제거를 확인합니다. 설치 폴더 안에서 직접 실행하면 사용 중인 자기 폴더를 삭제하지 못한 사례가 있으므로 [실기 기록](UNLOCKED_UI_REPORT.md)을 확인하세요. 제거가 완료되면 이 제품 메뉴와 설치기가 만든 신뢰 위치가 사라집니다. 기존 사용자 신뢰 위치와 외부에서 수정한 항목은 보존합니다. 업무 파일과 다른 추가 기능, 설치 폴더에 별도로 추가한 파일은 남습니다.

![제거 후 Excel을 정상 재시작한 실제 화면: 제품 메뉴와 누락 파일 경고 없음](images/10-uninstalled.jpg)

최신 실제 시험·복구 결과와 미실행 항목은 [RC8 검증 기록](RC8_COMPLETION_REPORT.md)을 확인하세요. [RC3 강건성 보고서](WINDOWS_ROBUSTNESS_REPORT.md)·[RC2](WINDOWS_TRUST_LOCATION_REPORT.md)·[RC1](WINDOWS_E2E_REPORT.md)은 이전 실행 기록입니다. Excel VBA 실행 이후 기본 실행 취소(Undo) 기록 보존은 보장하지 않습니다.
