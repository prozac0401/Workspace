...
try {
    ...
    foreach($id in $caseNames){
        ...
        $ra=$a.Range('A1:A4');$rb=$b.Range('H1:H4');$expected='A 4 / B 4 / 일치 3 / A 잔여 1 / B 잔여 1'
        switch($id){
            ...
            'SINGLE-ONLY' {$ra=$a.Range('A1');$rb=$b.Range('H2');$expected='A 1 / B 1 / 일치 0 / A 잔여 1 / B 잔여 1'}
            ...
            'HIDDEN-ROW' {$a.Rows.Item(3).Hidden=$true;$expected='A 3 / B 4 / 일치 2 / A 잔여 1 / B 잔여 2'}
            'HIDDEN-COLUMN' {$a.Cells.Clear();Fill $a @('A1','B1','C1','D1') @('one','one','two','three');$a.Columns.Item(3).Hidden=$true;$ra=$a.Range('A1:D1');$expected='A 3 / B 4 / 일치 2 / A 잔여 1 / B 잔여 2'}
            'AUTOFILTER-HEADER' {$a.Cells.Clear();Fill $a @('A1','A2','A3','A4','A5') @('heading','one','one','two','three');$null=$a.Range('A1:A5').AutoFilter(1,'<>two');$ra=$a.Range('A1:A5');$expected='A 3 / B 4 / 일치 2 / A 잔여 1 / B 잔여 2'}
            'TABLE-FILTER-TOTALS' {$a.Cells.Clear();Fill $a @('A1','A2','A3','A4','A5') @('heading','one','one','two','three');$lo=$a.ListObjects.Add(1,$a.Range('A1:A5'),[Type]::Missing,1);$lo.ShowTotals=$true;$null=$lo.Range.AutoFilter(1,'<>two');$ra=$lo.Range;$expected='A 3 / B 4 / 일치 2 / A 잔여 1 / B 잔여 2';Release-Com $lo}
        }
        ...
        if($id -eq 'SAME-RANGE-SNAPSHOT'){$a.Range('A4').Value2='changed';$rb=$ra;$b=$a;$expected='A 4 / B 4 / 일치 3 / A 잔여 1 / B 잔여 1'}
