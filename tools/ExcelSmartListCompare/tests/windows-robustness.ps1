...
function Caption {return [string]$e.CommandBars.Item('SLC_68A45C44_Toolbar').Controls.Item(2).Caption}
...
try {
 ...
 if($mode -eq 'repeat'){
  ...
  if($mode -eq 'capture'){Check 'captured exact count' ((Caption).Contains($count.ToString('#,##0')+'개 항목'))}
